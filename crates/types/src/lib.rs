...
///
/// let set = set! {Value::Number(1.into()), Value::Number(2.into()), Value::Number(3.into())};
/// ```
#[macro_export]
macro_rules! set {
	($($value:expr),* $(,)?) => {
...
mod tests {
    ...
    fn test_object_macro() {
        ...
        assert_eq!(
            obj2.0.get("first-name"),
            Some(&Value::String("John".to_string()))
        );
        assert_eq!(
            obj2.0.get("last-name"),
