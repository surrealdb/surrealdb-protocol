...
mod tests {
    ...
    fn test_duration_from_str() {
        ...
        assert_eq!(
            Duration::from_str("1m").unwrap(),
            Duration::from_mins(1).unwrap()
        );
        assert_eq!(
            Duration::from_str("1h").unwrap(),
            Duration::from_hours(1).unwrap()
        );
        assert_eq!(
            Duration::from_str("1d").unwrap(),
            Duration::from_days(1).unwrap()
        );
        assert_eq!(
            Duration::from_str("1w").unwrap(),
            Duration::from_weeks(1).unwrap()
        );
        assert_eq!(
            Duration::from_str("1y").unwrap(),
            Duration::new(365 * 24 * 60 * 60, 0)
        );

        // Test nanosecond units
        assert_eq!(
            Duration::from_str("1000ns").unwrap(),
            Duration::from_nanos(1000)
        );
        assert_eq!(
            Duration::from_str("1000ms").unwrap(),
