...
fn test_enum_tagged_with_tag() {
    ...
    assert_eq!(
        format!("{:?}", enum_kind),
        format!("Either([{kind_foo}, {kind_bar}])")
    );
...
fn test_enum_tagged_with_tag_variant_foo() {
    ...
    assert_eq!(
        value,
        Value::Object(object! { tag: Value::String("Foo".to_string()) })
    );
...
fn test_enum_tagged_with_tag_variant_lowercase() {
    ...
    assert_eq!(
        format!("{:?}", enum_kind),
        r#"Literal(Object({"tag": Literal(String("foo"))}))"#
    );
    ...
    assert!(!EnumTaggedWithTagLowercase::is_value(&Value::String(
        "Foo".to_string()
    )));
}
...
fn test_enum_tagged_with_tag_variant_uppercase() {
    ...
    assert!(!EnumTaggedWithTagUppercase::is_value(&Value::String(
        "Foo".to_string()
    )));
}
