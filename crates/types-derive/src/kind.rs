...
pub enum KindExpr {
    ...
    Parameterized { name: Ident, params: Vec<KindParam> },
...
impl KindExpr {
    ...
    fn parse_primary(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if name_str == "true" {
            return Ok(KindExpr::Literal(syn::Lit::Bool(syn::LitBool::new(
                true,
                name.span(),
            ))));
        ...
        if input.peek(Token![<]) {
            ...
            Ok(KindExpr::Parameterized { name, params })
        } else {
...
fn generate_kind_code(expr: &KindExpr) -> TokenStream2 {
    match expr {
        ...
        KindExpr::Parameterized { name, params } => generate_parameterized_kind(name, params),
        KindExpr::Union(variants) => {
