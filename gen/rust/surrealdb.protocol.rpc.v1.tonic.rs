...
pub mod surreal_db_service_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        > {
            ...
            let codec = tonic::codec::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/surrealdb.protocol.rpc.v1.SurrealDBService/GraphQl",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(
        ...
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                ...
                "/surrealdb.protocol.rpc.v1.SurrealDBService/GraphQl" => {
                    #[allow(non_camel_case_types)]
                    struct GraphQlSvc<T: SurrealDbService>(pub Arc<T>);
                    impl<
                        T: SurrealDbService,
                    > tonic::server::UnaryService<super::GraphQlRequest>
                    for GraphQlSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SurrealDbService>::graph_ql(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
