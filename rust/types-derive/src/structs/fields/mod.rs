        ...
        match self {
            Fields::Named(fields) => {
                ...
                match strategy {
                    ...
                    Strategy::TagKey { tag, variant } => With::Map(quote! {{
                    ...
                    Strategy::Value { variant: None } => With::Map(quote! {{
                        #(#map_retrievals)*
                        #final_ok
                    }}),
                }
            ...
            Fields::Unnamed(fields) => {
                ...
                if !fields.tuple && fields.fields.len() == 1 {
                    ...
                    match strategy {
                        ...
                        // For an enum, we check first if the variant matches, then decode
                        Strategy::Value { variant: Some(_) } => With::Value(quote! {{
                            if <#ty as SurrealValue>::is_value(&value) {
                                #retrieve
                            }
                        }}),
                        // For a struct, there's no other variants to try, so we just decode
                        Strategy::Value { variant: None } => With::Value(retrieve),
                    }
                } else {
                    ...
                    match strategy {
                        ...
                        // For a struct, there's no other variants to try, so we just decode
                        Strategy::Value { variant: None } => With::Arr(quote! {{
                            #retrieve_arr
                        }}),
                    }
                }
            ...
            Fields::Unit(attrs) => {
                match strategy {
                    ...
                    Strategy::Value {
                        ...
                        if let Some(UnitValue { is_value, .. }) = attrs.value.as_ref() {
                            With::Value(quote! {{
                                if #is_value {
                                    #ok
                                }
            ...
            Fields::Unnamed(fields) => {
                ...
                if !fields.tuple && fields.fields.len() == 1 {
                    ...
                    match strategy {
                        ...
                        Strategy::Value { .. } => With::Value(quote! {{
                            if #check {
                                return true;
                            }
                        }}),
