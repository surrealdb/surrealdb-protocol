...
impl FromFlatbuffers for RecordId {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        let table = input
            .table()
            .ok_or_else(|| anyhow::anyhow!("Missing table in RecordId"))?;
        let key = RecordIdKey::from_fb(
            input
                .id()
                .ok_or_else(|| anyhow::anyhow!("Missing id in RecordId"))?,
        )?;
        Ok(RecordId {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        match input.id_type() {
            ...
            proto_fb::RecordIdKeyType::String => {
                let key_value = input
                    .id_as_string()
                    .ok_or_else(|| anyhow::anyhow!("Expected String Id"))?;
            ...
            proto_fb::RecordIdKeyType::Array => {
                let key_value = input
                    .id_as_array()
                    .ok_or_else(|| anyhow::anyhow!("Expected Array Id"))?;
            ...
            proto_fb::RecordIdKeyType::Range => {
                let key_value = input
                    .id_as_range()
                    .ok_or_else(|| anyhow::anyhow!("Expected Range Id"))?;
...
impl FromFlatbuffers for RecordIdKeyRange {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        ...
        Ok(RecordIdKeyRange { start, end })
    }
}

impl ToFlatbuffers for Bound<RecordIdKey> {
    type Output<'bldr> = (
        proto_fb::RecordIdKeyBound,
    ...
    fn to_fb<'bldr>(
        ...
        Ok(match self {
            ...
            Bound::Unbounded => {
                ...
                (
                    proto_fb::RecordIdKeyBound::Unbounded,
                    Some(null_value.as_union_value()),
                )
            }
        })
    }
