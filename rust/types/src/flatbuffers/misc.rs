...
impl FromFlatbuffers for File {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        let bucket = input
            .bucket()
            .ok_or_else(|| anyhow::anyhow!("Missing bucket in File"))?;
        let key = input
            .key()
            .ok_or_else(|| anyhow::anyhow!("Missing key in File"))?;
        Ok(File {
...
impl FromFlatbuffers for Regex {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        let pattern = input
            .value()
            .ok_or_else(|| anyhow::anyhow!("Missing regex value"))?;
