    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        match input.geometry_type() {
            ...
            proto_fb::GeometryType::MultiLineString => {
                ...
                Ok(Geometry::MultiLine(geo::MultiLineString::from_fb(
                    multi_line_string,
                )?))
            }
            proto_fb::GeometryType::MultiPolygon => {
                ...
                Ok(Geometry::MultiPolygon(geo::MultiPolygon::from_fb(
                    multi_polygon,
                )?))
            }
...
impl FromFlatbuffers for geo::Polygon {
    ...
    fn from_fb(input: Self::Input<'_>) -> anyhow::Result<Self> {
        let exterior = input
            .exterior()
            .ok_or_else(|| anyhow::anyhow!("Missing exterior in Polygon"))?;
