...
fn test_simple_struct() {
    ...
    assert_eq!(
        format!("{:?}", person_kind),
        r#"Literal(Object({"age": Int, "name": String}))"#
    );
...
fn test_simple_single_field_tuple_struct() {
    ...
    assert_eq!(
        value,
        Value::Array(Array::from(vec![Value::String("Alice".to_string())]))
    );
...
fn test_unit_struct_with_value() {
    ...
    assert!(!UnitStructWithValue::is_value(
...
fn test_router_request() {
    ...
    assert_eq!(
        obj.get("method"),
        Some(&Value::String("request".to_string()))
    );
    assert_eq!(
        obj.get("params"),
        Some(&Value::String("request".to_string()))
...
fn test_test_optional() {
    ...
    let value = TestOptionalNoOption { id: 1 }.into_value();
