...
fn test_enum_tagged_variant_foo() {
    ...
    assert_eq!(
        value,
        Value::Object(object! { Foo: Value::Object(object! {}) })
    );
...
fn test_enum_tagged_variant_baz() {
    ...
    assert_eq!(
        value,
        Value::Object(object! { Baz: Value::String("baz".to_string()) })
    );
...
fn test_enum_tagged_variant_lowercase() {
    ...
    assert_eq!(
        format!("{:?}", enum_kind),
        r#"Literal(Object({"foo": Literal(Object({}))}))"#
    );
    ...
    assert!(!EnumTaggedVariantLowercase::is_value(&Value::String(
        "foo".to_string()
    )));
}
...
fn test_enum_tagged_variant_uppercase() {
    ...
    assert!(!EnumTaggedVariantUppercase::is_value(&Value::String(
        "FOO".to_string()
    )));
}
